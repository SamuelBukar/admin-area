...
export default function CreateForm() {
  ...
  // Refetch pages when component mounts to ensure we have latest templates
  useEffect(() => {
    refetchPages();
  }, [refetchPages]);
  ...
  useEffect(() => {
    ...
    if (templateId && templates.some(t => t.id === templateId) && !selectedIds.includes(templateId)) {
      setSelectedIds([templateId]);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams, templates.length]);
  ...
  const handleSave = (saveStatus: 'published' | 'draft' = status) => {
    ...
    linkTemplates.mutate(
      {
        ...
        pageData: { title, slug, description: description || undefined, status: saveStatus },
      },
      {
        onSuccess: () => {
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                disabled={linkTemplates.isPending || selectedIds.length === 0 || !title || !slug}
              >
